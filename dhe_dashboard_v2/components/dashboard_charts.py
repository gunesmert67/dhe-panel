...
def render_yearly_performance_chart(df_siparis, all_available_years, conversion_factor=1.0, sym="€"):
    ...
    # Başlık ve metrik seçici aynı satırda
    st.markdown('''
    <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 2rem; margin-bottom: 0.5rem;">
        <div>
            <div style="font-size: 1.1rem; font-weight: 700; color: var(--text-primary); letter-spacing: 0.05em;">YILLIK SATIŞ PERFORMANSI</div>
            <div style="font-size: 0.9rem; color: var(--text-secondary); margin-top: 4px;">Yılları açmak/kapatmak için yılların üzerine tıklayın</div>
        </div>
    </div>
    ...
    with col_metric:
        selected_metric = st.radio("METRİK", ["Ciro", "Kar"], index=0, horizontal=True, key="chart_metric_selector", label_visibility="collapsed")
