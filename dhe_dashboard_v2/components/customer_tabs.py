...
def render_details_tab(df_musteri, theme="light"):
    ...
    # Arama filtreleri
    if kisa_ad_arama:
        df_filtered = df_filtered[df_filtered["Kisa_Ad"].str.upper().str.contains(kisa_ad_arama.upper(), na=False)]
    
    if uzun_ad_arama:
        df_filtered = df_filtered[df_filtered["Uzun_Ad"].str.upper().str.contains(uzun_ad_arama.upper(), na=False)]
...
def render_products_tab(df_urun):
    ...
    if df_urun is not None and not df_urun.empty:
        ...
        kp1, kp2 = st.columns(2, gap="large")
