...
def calculate_effective_workdays(year: int, month: int = None, start_date: date = None, end_date: date = None, holidays: set = None):
    ...
    try:
        ...
        # --- TARİH TİPİ DÖNÜŞÜMLERİ (Timestamp -> date) ---
        # start_date ve end_date için pd.NaT kontrolü
        if pd.isna(start_date): # NaT veya None ise None yap
            start_date = None
        elif isinstance(start_date, (pd.Timestamp, datetime)):
            start_date = start_date.date()
            
        if pd.isna(end_date): # NaT veya None ise None yap
            end_date = None
