...
def load_data(_progress_callback=None) -> Dict[str, pd.DataFrame]:
    """Tüm verileri yükleyen ana fonksiyon (Paralel Veri Çekme)."""
    try:
        ...
        if _progress_callback: _progress_callback(0.10, "Veriler okunuyor (Paralel)...")
        
        # Paralel Çekilecek Verilerin Tanımı
        tasks = {
        ...
        # Yardımcı fonksiyon: safe_read_gsheet sarmalayıcı
        def fetch_wrapper(key, sheet_name, mapping):
            try:
                # safe_read_gsheet client ve spreadsheet istiyor
                # Her thread'de aynı client/spreadsheet kullanmak gspread için sorun olabilir mi?
                # Gspread client'ı HTTP tabanlı olduğu için genellikle thread-safe'dir.
                return key, safe_read_gsheet(client, spreadsheet, sheet_name, mapping)
            except Exception as e:
                logger.error(f"Paralel çekme hatası ({key}): {e}")
                return key, pd.DataFrame()

        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            future_to_sheet = {
                executor.submit(fetch_wrapper, k, v[0], v[1]): k 
        ...
        # 2. Yardımcı Veriler
        monthly_rates = get_monthly_rates_map_gsheet(spreadsheet)
        ...
        if _progress_callback: _progress_callback(0.80, "Veriler işleniyor...")

        # 3. Finansal İşleme
        df_teklif_processed = process_finance_dataframe(df_raw_teklif, "Teklif_No", monthly_rates, PERSONEL_MAP)
        # 0/0 Filtresi
        df_teklif_processed = df_teklif_processed[
            ~((df_teklif_processed["Tutar_EUR"] == 0) & (df_teklif_processed["Maliyet_EUR"] == 0))
        ]

        df_siparis_processed = process_finance_dataframe(df_raw_siparis, "Siparis_No", monthly_rates, PERSONEL_MAP)
        df_siparis_processed = df_siparis_processed[
            ~((df_siparis_processed["Tutar_EUR"] == 0) & (df_siparis_processed["Maliyet_EUR"] == 0))
        ]
        ...
        logger.info("[DHE_Data] Tüm veriler yüklendi")
        return {
